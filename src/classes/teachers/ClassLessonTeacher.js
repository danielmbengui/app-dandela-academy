...
export class ClassLessonTeacher {
    static COLLECTION = "LESSONS_TEACHER";
    static COLLECTION_TRANSLATE = "i18n";
    static NS_COLLECTION = `classes/lesson_teacher`;
    ...
    static ALL_CATEGORIES = [
        ClassLessonTeacher.CATEGORY.OFFICE
    ]
    static ALL_LEVELS = [
        ClassLessonTeacher.LEVEL.BEGINNER,
        ClassLessonTeacher.LEVEL.INTERMEDIATE,
        ClassLessonTeacher.LEVEL.COMPETENT,
        ClassLessonTeacher.LEVEL.ADVANCED,
        ClassLessonTeacher.LEVEL.EXPERT,
    ]
    static ALL_FORMATS = [
        ClassLessonTeacher.FORMAT.ONSITE,
        ClassLessonTeacher.FORMAT.ONLINE,
        ClassLessonTeacher.FORMAT.HYBRID,
    ]
    constructor({
        uid = "",
        uid_lesson = "",
        ...
        notes = [],
        photo_url = "",
        //        status = ClassLessonTeacher.STATUS.DRAFT,
        translate = {},
        translates = [],
    ...
    _normalizeCategory(category) {
        return Object.values(ClassLessonTeacher.CATEGORY).includes(category)
            ? category
            : ClassLessonTeacher.CATEGORY.UNKNOWN;
    }
    // uid
    get uid() {
        return this._uid;
    ...
    */
    ...
    clone() {
        ...
        return ClassLessonTeacher.makeLessonInstance(this._uid, {
            ...this.toJSON(),
            lesson: this._lesson,
    ...
    */
    ...
    _convertTranslatesFromFirestore(translatesObj = {}) {
        const translates = Object.values(translatesObj)?.map?.(trans => new ClassLessonTeacherTranslate(trans)) || [];
        return translates;
    }
    // ---------- Converter intégré ----------
    static _toJsDate(v) {
    ...
    static get converter() {
        return {
            ...
            fromFirestore(snapshot, options) {
                ...
                const translates = Object.values(data.translates)?.map?.(trans => new ClassLessonTeacherTranslate(trans));
                //console.log("uid lesson",uid, )
                //console.log("translate ClassLessonTeacher", Object.values(data.translates))
                return ClassLessonTeacher.makeLessonInstance(uid, { ...data, start_date, end_date, created_time, last_edit_time, translates });
            },
        };
    }
    // ---------- Helpers Firestore ----------
    static async alreadyExist(_uid) {
        const q = query(
            collection(firestore, ClassLessonTeacher.COLLECTION),
            where("uid", "==", _uid)
        );
    ...
    static async get(uid, lang = defaultLanguage) {
        ...
        if (snap.exists()) {
            ...
            //const translates = await ClassLessonTeacherTranslate.fetchListFromFirestore(uid);
    ...
    static async getByName(_name) {
        ...
        const q = query(
            collection(firestore, ClassLessonTeacher.COLLECTION).withConverter(ClassLessonTeacher.converter),
    ...
    async createFirestore() {
        ...
        //data.uid = newRef.id;
        //const model = data instanceof ClassLessonTeacher ? data : new ClassLessonTeacher({ ...data });
        //model.uid = newRef.id;()
    ...
    async updateFirestore() {
        try {
            // const ref = ClassLessonTeacher.docRef(id);
    ...
    static async fetchFromFirestore(uid = "", lang = defaultLanguage) {
        try {
            ...
            //const _translate = await ClassLessonTeacherTranslate.fetchFromFirestore(uid, lang);
    ...
    static async fetchFromFirestoreNeme(_name) {
        try {
            if (!uid) throw new Error("UID is required to get school.");
            return await ClassLessonTeacher.getByName(_name);
        } catch (error) {
            console.log("ERROR", error?.message || error);
            return null;
        }
...
export class ClassLessonTeacherTranslate {
    ...
    */
    ...
    clone() {
        return ClassLessonTeacherTranslate.makeLessonTranslateInstance(this._uid_lesson, this.toJSON());
        //return new ClassUser(this.toJSON());
    }
    ...
    static makeLessonTranslateInstance(uid_lesson, data = {}) {
        return new ClassLessonTeacherTranslate({ uid_lesson, ...data });
    }
    static get converter() {
        return {
            toFirestore(lessonInstance) {
            ...
            fromFirestore(snapshot, options) {
                ...
                const data = snapshot.data(options) || {};
                var created_time = ClassLessonTeacherTranslate._toJsDate(data.created_time);
                var last_edit_time = ClassLessonTeacherTranslate._toJsDate(data.last_edit_time);
                return ClassLessonTeacherTranslate.makeLessonTranslateInstance(lang, { ...data, created_time, last_edit_time, lang });
            },
        };
    }
    // ---------- Helpers Firestore ----------
    ...
    static colRef(uidLesson = "") {
        return collection(firestore, ClassLessonTeacher.COLLECTION, uidLesson, ClassLessonTeacherTranslate.COLLECTION).withConverter(this.converter);
    }
    static docRef(uidLesson = "", lang) {
        return doc(firestore, ClassLessonTeacher.COLLECTION, uidLesson, ClassLessonTeacherTranslate.COLLECTION, lang).withConverter(this.converter);
    }
    static async count() {
        //const coll = collection(firestore, ClassUser.COLLECTION);
        const coll = this.colRef();
