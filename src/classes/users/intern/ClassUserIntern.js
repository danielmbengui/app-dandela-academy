...
export class ClassUserIntern extends ClassUser {
    ...
    static ALL_ROLES = [
        ClassUserIntern.ROLE.SUPER_ADMIN,
        ClassUserIntern.ROLE.ADMIN,
        ClassUserIntern.ROLE.TEAM,
        ClassUserIntern.ROLE.TUTOR,
    ];
    ...
    constructor({
        ...
        status = ClassUserIntern.STATUS.CREATED,
    } = {}) {
        super({
            uid,
            type: ClassUserIntern.TYPE.INTERN,
            role,
            email,
            first_name,
            last_name,
    ...
    static get converter() {
        return {
            ...
            fromFirestore(snapshot, options) {
                ...
                return new ClassUserIntern({
                    ...data,
                    uid: data.uid ?? snapshot.id,
                    created_time: ClassUserIntern._toJsDate(data.created_time) ?? new Date(),
                    last_edit_time: ClassUserIntern._toJsDate(data.last_edit_time) ?? new Date(),
                    birthday: ClassUserIntern._toJsDate(data.birthday) ?? new Date(),
                    email: data.email,
    ...
    // ---------- Helpers Firestore ----------    
    static colRef() {
        return collection(firestore, ClassUserIntern.COLLECTION).withConverter(ClassUserIntern.converter);
    }

    static docRef(id) {
        return doc(firestore, ClassUserIntern.COLLECTION, id).withConverter(ClassUserIntern.converter);
    }

    static async count() {
        const q = query(ClassUserIntern.colRef(), where("type", "==", ClassUserIntern.TYPE.INTERN));
    ...
    static async get(id) {
        const snap = await getDoc(ClassUserIntern.docRef(id));
        if (snap.exists()) {
    ...
    static async list(constraints = []) {
        const q = constraints.length ? query(ClassUserIntern.colRef(),where("type", "==", ClassUserIntern.TYPE.INTERN), ...constraints) : query(ClassUserIntern.colRef(),where("type", "==", ClassUserIntern.TYPE.INTERN));
        const qSnap = await getDocs(q);
        const users = [];
    ...
    static async create(data = {}, { useServerTimestamps = true } = {}) {
        const newRef = doc(ClassUserIntern.colRef()); // id auto
        const model = data instanceof ClassUserIntern ? data : new ClassUserIntern({ uid: newRef.id, ...data });
    ...
    static async update(id, patch = {}, { bumpEditTime = true } = {}) {
        const ref = ClassUserIntern.docRef(id);
    ...
    static async remove(id) {
        await deleteDoc(ClassUserIntern.docRef(id));
        return true;
    }
    ...
    static async fetchFromFirestore(uid) {
        try {
            ...
            return await ClassUserIntern.get(uid);
        } catch (error) {
    ...
    static async fetchListFromFirestore(constraints = []) {
        try {
            //if (!uid) throw new Error("UID is required to get module.");
            return await ClassUserIntern.list(constraints);
        } catch (error) {
            console.log("ERROR", error?.message || error);
            return null;
        }
