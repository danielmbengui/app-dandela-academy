...
export class ClassUserSuperAdmin extends ClassUserIntern {
    constructor({
        ...
        status = ClassUserSuperAdmin.STATUS.CREATED,
    } = {}) {
        super({
            uid,
            type: ClassUserSuperAdmin.TYPE.INTERN,
            role: ClassUserSuperAdmin.ROLE.SUPER_ADMIN,
            email,
            first_name,
            last_name,
            email_verified,
    ...
    static get converter() {
        return {
            ...
            fromFirestore(snapshot, options) {
                ...
                return new ClassUserSuperAdmin({
                    ...data,
                    uid: data.uid ?? snapshot.id,
                    created_time: ClassUserSuperAdmin._toJsDate(data.created_time) ?? new Date(),
                    last_edit_time: ClassUserSuperAdmin._toJsDate(data.last_edit_time) ?? new Date(),
                    birthday: ClassUserSuperAdmin._toJsDate(data.birthday) ?? new Date(),
                    email: data.email,
    ...
    // ---------- Helpers Firestore ----------    
    static colRef() {
        return collection(firestore, ClassUserSuperAdmin.COLLECTION).withConverter(ClassUserSuperAdmin.converter);
    }

    static docRef(id) {
        return doc(firestore, ClassUserSuperAdmin.COLLECTION, id).withConverter(ClassUserSuperAdmin.converter);
    }

    static async count() {
        const q = query(ClassUserSuperAdmin.colRef(), where("type", "==", ClassUserSuperAdmin.TYPE.INTERN), where("role", "==", ClassUserSuperAdmin.ROLE.SUPER_ADMIN));
    ...
    static async get(id) {
        const snap = await getDoc(ClassUserSuperAdmin.docRef(id));
        if (snap.exists()) {
    ...
    static async list(constraints = []) {
        const q = constraints.length ? query(ClassUserSuperAdmin.colRef(), where("type", "==", ClassUserSuperAdmin.TYPE.INTERN), where("role", "==", ClassUserSuperAdmin.ROLE.SUPER_ADMIN), ...constraints) : query(ClassUserSuperAdmin.colRef(), where("type", "==", ClassUserSuperAdmin.TYPE.INTERN), where("role", "==", ClassUserSuperAdmin.ROLE.SUPER_ADMIN));
        const qSnap = await getDocs(q);
        const users = [];
    ...
    static async create(data = {}, { useServerTimestamps = true } = {}) {
        const newRef = doc(ClassUserSuperAdmin.colRef()); // id auto
        const model = data instanceof ClassUserSuperAdmin ? data : new ClassUserSuperAdmin({ uid: newRef.id, ...data });
    ...
    static async update(id, patch = {}, { bumpEditTime = true } = {}) {
        const ref = ClassUserSuperAdmin.docRef(id);
    ...
    static async remove(id) {
        await deleteDoc(ClassUserSuperAdmin.docRef(id));
        return true;
    }
    ...
    static async fetchFromFirestore(uid) {
        try {
            ...
            return await ClassUserSuperAdmin.get(uid);
        } catch (error) {
    ...
    static async fetchListFromFirestore(constraints = []) {
        try {
            //if (!uid) throw new Error("UID is required to get module.");
            return await ClassUserSuperAdmin.list(constraints);
        } catch (error) {
            console.log("ERROR", error?.message || error);
            return null;
        }
