...
export class ClassUserTeam extends ClassUserIntern {
    constructor({
        ...
        status = ClassUserTeam.STATUS.CREATED,
    } = {}) {
        super({
            uid,
            type: ClassUserTeam.TYPE.INTERN,
            role: ClassUserTeam.ROLE.TEAM,
            email,
            first_name,
            last_name,
            email_verified,
    ...
    static get converter() {
        return {
            ...
            fromFirestore(snapshot, options) {
                ...
                return new ClassUserTeam({
                    ...data,
                    uid: data.uid ?? snapshot.id,
                    created_time: ClassUserTeam._toJsDate(data.created_time) ?? new Date(),
                    last_edit_time: ClassUserTeam._toJsDate(data.last_edit_time) ?? new Date(),
                    birthday: ClassUserTeam._toJsDate(data.birthday) ?? new Date(),
                    email: data.email,
    ...
    // ---------- Helpers Firestore ----------    
    static colRef() {
        return collection(firestore, ClassUserTeam.COLLECTION).withConverter(ClassUserTeam.converter);
    }

    static docRef(id) {
        return doc(firestore, ClassUserTeam.COLLECTION, id).withConverter(ClassUserTeam.converter);
    }

    static async count() {
        const q = query(ClassUserTeam.colRef(), where("type", "==", ClassUserTeam.TYPE.INTERN), where("role", "==", ClassUserTeam.ROLE.TEAM));
    ...
    static async get(id) {
        const snap = await getDoc(ClassUserTeam.docRef(id));
        if (snap.exists()) {
    ...
    static async list(constraints = []) {
        const q = constraints.length ? query(ClassUserTeam.colRef(), where("type", "==", ClassUserTeam.TYPE.INTERN), where("role", "==", ClassUserTeam.ROLE.TEAM), ...constraints) : query(ClassUserTeam.colRef(), where("type", "==", ClassUserTeam.TYPE.INTERN), where("role", "==", ClassUserTeam.ROLE.TEAM));
        const qSnap = await getDocs(q);
        const users = [];
    ...
    static async create(data = {}, { useServerTimestamps = true } = {}) {
        const newRef = doc(ClassUserTeam.colRef()); // id auto
        const model = data instanceof ClassUserTeam ? data : new ClassUserTeam({ uid: newRef.id, ...data });
    ...
    static async update(id, patch = {}, { bumpEditTime = true } = {}) {
        const ref = ClassUserTeam.docRef(id);
    ...
    static async remove(id) {
        await deleteDoc(ClassUserTeam.docRef(id));
        return true;
    }
    ...
    static async fetchFromFirestore(uid) {
        try {
            ...
            return await ClassUserTeam.get(uid);
        } catch (error) {
    ...
    static async fetchListFromFirestore(constraints = []) {
        try {
            //if (!uid) throw new Error("UID is required to get module.");
            return await ClassUserTeam.list(constraints);
        } catch (error) {
            console.log("ERROR", error?.message || error);
            return null;
        }
