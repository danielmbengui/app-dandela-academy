...
export class ClassUserAdmin extends ClassUserIntern {
    constructor({
        ...
        status = ClassUserAdmin.STATUS.CREATED,
    } = {}) {
        super({
            uid,
            type: ClassUserAdmin.TYPE.INTERN,
            role: ClassUserAdmin.ROLE.ADMIN,
            email,
            first_name,
            last_name,
            email_verified,
    ...
    static get converter() {
        return {
            ...
            fromFirestore(snapshot, options) {
                ...
                return new ClassUserAdmin({
                    ...data,
                    uid: data.uid ?? snapshot.id,
                    created_time: ClassUserAdmin._toJsDate(data.created_time) ?? new Date(),
                    last_edit_time: ClassUserAdmin._toJsDate(data.last_edit_time) ?? new Date(),
                    birthday: ClassUserAdmin._toJsDate(data.birthday) ?? new Date(),
                    email: data.email,
    ...
    */
    ...
    // ---------- Helpers Firestore ----------    
    static colRef() {
        return collection(firestore, ClassUserAdmin.COLLECTION).withConverter(this.converter);
    }

    static docRef(id) {
        return doc(firestore, ClassUserAdmin.COLLECTION, id).withConverter(this.converter);
    }

    static async count() {
        const q = query(ClassUserAdmin.colRef(), where("type", "==", ClassUserAdmin.TYPE.INTERN), where("role", "==", ClassUserAdmin.ROLE.ADMIN));
    ...
    static async get(id) {
        const snap = await getDoc(ClassUserAdmin.docRef(id));
        if (snap.exists()) {
    ...
    static async list(constraints = []) {
        const q = constraints.length ? query(ClassUserAdmin.colRef(),where("type", "==", ClassUserAdmin.TYPE.INTERN), where("role", "==", ClassUserAdmin.ROLE.ADMIN), ...constraints) : query(ClassUserAdmin.colRef(),where("type", "==", ClassUserAdmin.TYPE.INTERN), where("role", "==", ClassUserAdmin.ROLE.ADMIN));
        const qSnap = await getDocs(q);
        const users = [];
    ...
    static async create(data = {}, { useServerTimestamps = true } = {}) {
        const newRef = doc(ClassUserAdmin.colRef()); // id auto
        const model = data instanceof ClassUserAdmin ? data : new ClassUserAdmin({ uid: newRef.id, ...data });
    ...
    static async update(id, patch = {}, { bumpEditTime = true } = {}) {
        const ref = ClassUserAdmin.docRef(id);
    ...
    static async remove(id) {
        await deleteDoc(ClassUserAdmin.docRef(id));
        return true;
    }
    ...
    static async fetchFromFirestore(uid) {
        try {
            ...
            return await ClassUserAdmin.get(uid);
        } catch (error) {
    ...
    static async fetchListFromFirestore(constraints = []) {
        try {
            //if (!uid) throw new Error("UID is required to get module.");
            return await ClassUserAdmin.list(constraints);
        } catch (error) {
            console.log("ERROR", error?.message || error);
            return null;
        }
