...
export class ClassUserStudent extends ClassUserExtern {
  constructor({
      ...
      status = ClassUserStudent.STATUS.CREATED,
      goals = "",
      level = '',
      okay_whatsapp = false,
      ...
      super({
          uid,
          type: ClassUserStudent.TYPE.EXTERN,
          role:ClassUserStudent.ROLE.STUDENT,
          email,
          first_name,
          last_name,
          email_verified,
  ...
  static get converter() {
      return {
          ...
          fromFirestore(snapshot, options) {
              ...
              return new ClassUserStudent({
                  ...data,
                  uid: data.uid ?? snapshot.id,
                  created_time: ClassUserStudent._toJsDate(data.created_time) ?? new Date(),
                  last_edit_time: ClassUserStudent._toJsDate(data.last_edit_time) ?? new Date(),
                  birthday: ClassUserStudent._toJsDate(data.birthday) ?? new Date(),
                  email: data.email,
  ...
  // ---------- Helpers Firestore ----------    
  static colRef() {
      return collection(firestore, ClassUserStudent.COLLECTION).withConverter(ClassUserStudent.converter);
  }

  static docRef(id) {
      return doc(firestore, ClassUserStudent.COLLECTION, id).withConverter(ClassUserStudent.converter);
  }

  static async count() {
      const q = query(ClassUserStudent.colRef(), where("type", "==", ClassUserStudent.TYPE.EXTERN), where("role", "==", ClassUserStudent.ROLE.STUDENT));
  ...
  static async get(id) {
      const snap = await getDoc(ClassUserStudent.docRef(id));
      if (snap.exists()) {
  ...
  static async list(constraints = []) {
      const q = constraints.length ? query(ClassUserStudent.colRef(), where("type", "==", ClassUserStudent.TYPE.EXTERN), where("role", "==", ClassUserStudent.ROLE.STUDENT),...constraints) : query(ClassUserStudent.colRef(),where("type", "==", ClassUserStudent.TYPE.EXTERN), where("role", "==", ClassUserStudent.ROLE.STUDENT));
      const qSnap = await getDocs(q);
      const users = [];
  ...
  static async create(data = {}, { useServerTimestamps = true } = {}) {
      const newRef = doc(ClassUserStudent.colRef()); // id auto
      const model = data instanceof ClassUserStudent ? data : new ClassUserStudent({ uid: newRef.id, ...data });
  ...
  static async update(id, patch = {}, { bumpEditTime = true } = {}) {
      const ref = ClassUserStudent.docRef(id);
  ...
  static async remove(id) {
      await deleteDoc(ClassUserStudent.docRef(id));
      return true;
  }
  ...
  static async fetchFromFirestore(uid) {
      try {
          ...
          return await ClassUserStudent.get(uid);
      } catch (error) {
  ...
  static async fetchListFromFirestore(constraints = []) {
      try {
          //if (!uid) throw new Error("UID is required to get module.");
          return await ClassUserStudent.list(constraints);
      } catch (error) {
          console.log("ERROR", error?.message || error);
          return null;
      }
