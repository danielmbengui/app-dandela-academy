...
const Card = () => {
  ...
  useEffect(() => {
    async function init() {
      const count = await ClassUserWaitingList.count();
      setCountUsers(count);
    }
    init();
  })
  return (
    <div className="group relative flex w-70 lg:w-80 flex-col rounded-xl bg-slate-950 p-4 shadow-2xl transition-all duration-300 hover:scale-[1.02] hover:shadow-indigo-500/20">
      ...
      <div className="relative">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-white">Liste attente</h3>
          </div>
        ...
        <div className="mb-4 grid grid-cols-2 gap-4">
          ...
          <div className="rounded-lg bg-slate-900/50 p-3">
            <p className="text-xs font-medium text-slate-400">Total liste attente</p>
            <p className="text-lg font-semibold text-white">{countUsers}</p>
            <span className="text-xs font-medium text-emerald-500">+8.1%</span>
          </div>
          <div className="rounded-lg bg-slate-900/50 p-3">
        ...
        <div className="mb-4 h-24 w-full overflow-hidden rounded-lg bg-slate-900/50 p-3">
          <div className="flex h-full w-full items-end justify-between gap-1">
            <div className="h-[40%] w-3 rounded-sm bg-cyan-600/30">
              <div className="h-[60%] w-full rounded-sm bg-secondaryColor transition-all duration-300" />
            </div>
...
const DashboardMain = ({ user = null }) => {
  return (
    <>
      {
        //<FirstConnexion />
      }
      <SubHeaderDashboard user={user} />
      {
        //user instanceof ClassUserExtern && <AdminFeedbacks />
      }
    </>
  );
};
