...
import OneStatComponent from "../OneStatComponent";
...
function ViewScoreComponent({stat=null, chapter = null }) {
    // if (!chapter) return;
    const { t } = useTranslation([ClassUserStat.NS_COLLECTION]);
    ...
    return (<OneStatComponent stat={stat} />)
}
function ViewAverageComponent({ stat = null, chapter = null, lesson = null }) {
    ...
    const { hasMaxStats, countScore, countQuestions, percent, percentWrong, totalDuration, status,
        colorBest, colorWorst, colorMax,
        countScoreCorrect, countScoreWrong,
        statsFiltered, chaptersFiltered, statusTab, countQuiz, } = useMemo(() => {
            ...
            const countQuiz = stat?.answers?.length;
            const countScore = stat?.score;
            const countQuestions = stat?.answers.length || 0;
            const countScoreCorrect = countScore;
            const countScoreWrong = countQuestions - countScore;
            const hasMaxStats = countScore === countQuestions;

            const percent = stat?.percentage;
            const percentWrong = 100 - percent;
            const totalDuration = stat?.duration;
            const status = stat?.status;
            const STATUS_CONFIG = ClassUserStat.STATUS_CONFIG || [];
            const colorBest = STATUS_CONFIG[ClassUserStat.STATUS.EXCELLENT];
            const colorWorst = STATUS_CONFIG[ClassUserStat.STATUS.NOT_GOOD];
            const colorMax = STATUS_CONFIG[ClassUserStat.STATUS.MAX];
            return {
                statsFiltered,
                chaptersFiltered,
                statusTab,
        ...
        }, [stat, chapter, lesson, stats, chapters]);
    ...
    return (<Stack spacing={1} sx={{ py: 1 }} alignItems={'start'}>
        ...
        }
        {
            hasStats && <Grid direction="row" container spacing={1} sx={{
                ...
                <Grid size={{ xs: 12, sm: 'auto' }} sx={{ height: '100%', background: '' }}>
                    <Stack justifyContent={'center'} alignItems={'center'} spacing={1}
                        ...
                        
                        {
                            hasMaxStats ?
                                <>
                                <Chip size="small" label={t(colorMax.label)} sx={{
                            fontWeight: 950,
                            bgcolor: colorMax.background_bubble,
                            color: colorMax.color_icon,
                            border: colorMax.border,
                        }} />
                                <EmojiEventsIcon sx={{ color: colorMax?.color_icon, fontSize: '30px' }} />
                                </> :
                                <>
                            ...
                            border: colorAverage.border,
                                ...
                                <IconCharts height={30} width={30} color={colorAverage?.color_icon} />
                                </>
                        }
                        ...
                        <CircularProgressChapter
                            ...
                            size="large"
                            status={hasMaxStats ? colorMax.label : 'average'}
                        />
                    </Stack>
                </Grid>
                <Grid size={'grow'}>
                ...
                <Grid size={{ xs: 12, sm: 'auto' }} key={`${status}`}>
                                <Stack justifyContent={'center'} alignItems={'center'} spacing={1}
                                    ...
                                    <Chip size="small" label={t('result.filter-trues')} sx={{
                                        fontWeight: 950,
                                    ...
                                    <CircularProgressChapter
                                        score={countScoreCorrect}
                                        questions={countQuestions}
                                        percent={percent}
                                        duration={totalDuration}
                                        showDuration={false}
                                        size="medium"
                                        status={colorBest.label}
                                    />
                                </Stack>
                            </Grid>
                            <Grid size={{ xs: 12, sm: 'auto' }}>
                                <Stack justifyContent={'center'} alignItems={'center'} spacing={1}
                                    ...
                                    <Chip size="small" label={t('result.filter-falses')} sx={{
                                        fontWeight: 950,
                                        bgcolor: colorWorst.background_bubble,
                                        color: colorWorst.color_icon,
                                        border: colorWorst.border,
                                    }} />
                                    <CircularProgressChapter
                                        score={countScoreWrong}
                                        questions={countQuestions}
                                        percent={percentWrong}
                                        duration={totalDuration}
                                        showDuration={false}
                                        size="medium"
                                        status={colorWorst.label}
                                    />
                                </Stack>
                            </Grid>
                        </Grid>
...
function AverageComponent({ stat = null, chapter = null }) {
    //if (!chapter) return;
    const router = useRouter();
    const STATUS_CONFIG = ClassUserStat.STATUS_CONFIG || [];
    const { t } = useTranslation([ClassUserStat.NS_COLLECTION]);
    const { getOneStatIndex, getGlobalCountQuiz, setUidStat, isLoading: isLoadingStats, stats, getGlobalScore, getGlobalDuration, getGlobalCountQuestions, getGlobalPercent, getBestStat, getWorstStat, getGlobalCountLesson, getGlobalCountChapters, countHourTotalLessons } = useStat();
    const { statsFiltered,
...
export default function StatsOneStatListComponent({ stats = [], isOpenDetails = false, setIsOpenDetails = null, viewMode = ClassUserStat.VIEW_MODE_SCORE }) {
    const isViewScore = viewMode === ClassUserStat.VIEW_MODE_SCORE;
    const { t } = useTranslation([ClassUserStat.NS_COLLECTION]);
    const { lesson, } = useLesson();
    const { chapter, chapters } = useChapter();
    const { getBestStat, stat } = useStat();

    return (<Grid container spacing={1} sx={{ background: '', width: '100%', }}>

        {
            isViewScore && <ViewScoreComponent stat={stat} chapter={chapter} />
        }

        <Grid size={12}>
            {
                !isViewScore && <ViewAverageComponent stat={stat} lesson={lesson} chapter={chapter} />
            }
        </Grid>
    </Grid>)
}
