...
import { NS_TERMS_PRIVACY, NS_TERMS_USAGE } from "@/contexts/i18n/settings";
...
export default function UsageComponent() {
    const { t } = useTranslation([NS_TERMS_USAGE]);
    const chaptersTranslate = t('chapters', { returnObjects: true }) || [];
    const { school } = useSchool();
    return (<div className="page">
        <main className="container">
            <Typography sx={{ color: 'var(--font-color)' }}>{t('title')}{" / "}<Link href={PAGE_TERMS_PRIVACY} style={{ color: 'var(--primary)', fontWeight: 300 }}>{t('title-privacy')}</Link></Typography>
            {/* HERO */}
