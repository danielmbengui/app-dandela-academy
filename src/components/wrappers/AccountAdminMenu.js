  ...
  return (
    <>
      <Box sx={{ display: 'flex', alignItems: 'center', textAlign: 'center' }}>
        <Stack
          ...
          sx={{
            ...
            borderRadius: '24px',
            cursor: 'pointer',
            background: 'linear-gradient(135deg, var(--admin-shadow-xs) 0%, var(--admin-shadow) 100%)',
            transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
            '&:hover': {
              background: 'linear-gradient(135deg, var(--admin-shadow-sm) 0%, var(--admin-shadow-xs) 100%)',
              borderColor: 'var(--admin)',
              transform: 'translateY(-1px)',
              boxShadow: '0 4px 12px var(--admin-shadow-md)',
            },
            '&:active': {
        ...
        >
          ...
          <Stack>
            <Typography variant={'string'} noWrap fontSize={'13px'} sx={{ fontWeight: 600, color: 'var(--admin)' }}>
      ...
      <Menu
        ...
        onClose={handleClose}
        onClick={(e) => {
          if (e.target.closest('[data-lang-menu]') || e.target.closest('#lang-submenu') || e.target.closest('[data-theme-menu]') || e.target.closest('#theme-submenu')) {
            return;
          }
          handleClose();
        }}
          ...
          paper: {
            ...
            sx: {
              ...
              '& .MuiMenuItem-root': {
                ...
                '&:hover': {
                  backgroundColor: 'var(--admin-shadow-xs)',
                  color: 'var(--admin)',
                  transform: 'translateX(4px)',
                },
                '&.Mui-selected': {
                  backgroundColor: 'var(--admin-shadow-sm)',
                  color: 'var(--admin)',
                  fontWeight: 600,
                  '&:hover': {
                    backgroundColor: 'var(--admin-shadow-md)',
                  }
                }
              },
              '& .MuiDivider-root': {
      ...
      >
        ...
        <MenuItem
          ...
          sx={{
            '& .MuiListItemIcon-root': {
              minWidth: 40,
              color: path?.includes(PAGE_DASHBOARD_PROFILE) ? 'var(--admin)' : 'var(--font-color)',
            }
          }}
        >
          <ListItemIcon>
        ...
        <MenuItem
          ...
          sx={{
            '& .MuiListItemIcon-root': {
              minWidth: 40,
              color: path?.includes(PAGE_SETTINGS) ? 'var(--admin)' : 'var(--font-color)',
            }
          }}
        >
          <ListItemIcon>
        ...
        <MenuItem
          ...
          sx={{
            '& .MuiListItemIcon-root': {
              minWidth: 40,
              color: langMenuOpen ? 'var(--admin)' : 'var(--font-color)',
            },
            backgroundColor: langMenuOpen ? 'var(--admin-shadow)' : 'transparent',
          }}
        >
          <ListItemIcon>
          ...
          <Stack direction={'row'} alignItems={'center'} spacing={1} sx={{ flex: 1, justifyContent: 'space-between' }}>
            ...
            {currentLanguage && (
              <Stack direction={'row'} alignItems={'center'} spacing={0.75}>
                ...
                <Typography sx={{
                  fontSize: '0.8rem',
                  color: langMenuOpen ? 'var(--admin)' : 'var(--grey-light)',
                  fontWeight: 500
                }}>
        ...
        <MenuItem
          ...
          sx={{
            '& .MuiListItemIcon-root': {
              minWidth: 40,
              color: themeMenuOpen ? 'var(--admin)' : 'var(--font-color)',
            },
            backgroundColor: themeMenuOpen ? 'var(--admin-shadow)' : 'transparent',
          }}
        >
          <ListItemIcon>
            {currentTheme.icon}
          ...
          <Stack direction={'row'} alignItems={'center'} spacing={1} sx={{ flex: 1, justifyContent: 'space-between' }}>
            ...
            {currentTheme && (
              <Typography sx={{
                fontSize: '0.8rem',
                color: themeMenuOpen ? 'var(--admin)' : 'var(--grey-light)',
                fontWeight: 500
              }}>
                {currentTheme.label}
              </Typography>
        ...
        >
          {themes.map((theme) => (
            ...
            >
              <Stack direction={'row'} alignItems={'center'} spacing={1.5} sx={{ width: '100%' }}>
                <Box sx={{ color: modeApp === theme.id ? 'var(--admin)' : 'var(--font-color)' }}>
                  {theme.icon}
                </Box>
                <Typography sx={{
                  fontSize: '0.9rem',
