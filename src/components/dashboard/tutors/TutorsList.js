...
const TutorsList = () => {
  ...
  return (
    <div className="p-15px md:px-10 md:py-50px mb-30px bg-whiteColor dark:bg-whiteColor-dark shadow-accordion dark:shadow-accordion-dark rounded-5 max-h-[100%]">
      ...
      <ul>
        {instructors?.map(
          ({ id, name, avatar, reviews, students, courses }, idx) => (
            ...
            >
              {/* avatar */}
              <div className="max-w-full md:max-w-20px pr-10px">
                <Image src={avatar} width={40} height={40} alt="" className="w-full rounded-full" />
              </div>
              {/* details */}
