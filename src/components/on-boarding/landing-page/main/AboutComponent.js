...
import about1 from "@/assets/images/home/about-4.png";
...
import { IMAGE_HOME_ABOUT, IMAGE_HOME_STUDENT, IMAGE_HOME_STUDENT_WOMAN } from "@/libs/constants/constants_images";
const AboutComponent = ({ children, image, hideCounter }) => {
    const { t } = useTranslation([NS_HOME]);
    const { title, subtitle, bubble, goals, tag } = t(`about`, { returnObjects: true });
    return (
        <section className={`container pb-100px`}>
            <div id='about'>
                {/* about section   */}
                <div className="grid grid-cols-1 lg:grid-cols-2 pt-100px gap-x-30px">
                    {/* about left  */}
                    <div
                    ...
                    <div data-aos="fade-up">
                        ...
                        <ul className="space-y-[14px]">
                            {
                                goals.map((goal, i) => {
                                    return (<li key={`${goal}-${i}`} className="flex items-center group">
