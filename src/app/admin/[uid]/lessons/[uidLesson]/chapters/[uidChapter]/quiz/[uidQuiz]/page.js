...
/** Normalise une question du quiz (ClassLessonChapterQuestion) pour le formulaire */
function normalizeQuestionForForm(q) {
  const trans = q.getTranslate
    ? q.getTranslate(defaultLanguage)
    : (Array.isArray(q.translates) &&
        q.translates.find((t) => t.lang === defaultLanguage)) ||
      (Array.isArray(q.translates) && q.translates[0]);
  return {
    uid_intern: q.uid_intern,
    question: (trans && trans.question) || q.question || "",
    proposals: (trans && trans.proposals) || q.proposals || [],
    answer: (trans && trans.answer) || q.answer || {},
...
export default function EditQuizPage() {
  ...
  const { uid: uidUser, uidLesson, uidChapter, uidQuiz } = params;
  const { user } = useAuth();
  ...
  // Charger les questions du quiz existant
  useEffect(() => {
    if (!chapter?.quiz?.questions?.length || initialized) return;
    const quiz = chapter.quiz;
    if (String(quiz.uid_intern) !== String(uidQuiz)) return;
    const normalized = quiz.questions.map(normalizeQuestionForForm);
    setQuestions(normalized);
    setInitialized(true);
  }, [chapter, uidQuiz, initialized]);
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: t("error-update-quiz", { ns: NS_ADMIN_CHAPTERS }),
        severity: "error",
      });
    } finally {
  ...
  const handleSaveQuiz = async () => {
    ...
    try {
      ...
      const quiz = new ClassLessonChapterQuiz({
        uid_intern: chapter.quiz?.uid_intern ?? uidQuiz,
  ...
  const quizExists = chapter?.quiz && String(chapter.quiz.uid_intern) === String(uidQuiz);
    ...
    return (
      ...
      >
        <Stack spacing={2} sx={{ width: "100%", maxWidth: 1024 }}>
          ...
          <Link
            href={PAGE_ADMIN_CHAPTER_QUIZ(uidUser, uidLesson, uidChapter)}
    ...
    <AdminPageWrapper
      titles={[
          ...
          : []),
        {
          ...
          url: PAGE_ADMIN_CHAPTER_QUIZ(uidUser, uidLesson, uidChapter),
        },
        { name: t("edit-quiz", { ns: NS_ADMIN_CHAPTERS }) },
      ]}
      isAuthorized={isAuthorized}
    ...
    >
      <Stack spacing={2} sx={{ width: "100%", maxWidth: 1024 }}>
        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
          ...
          />
          <ButtonConfirm
            label={t("update-quiz", { ns: NS_ADMIN_CHAPTERS })}
            isAdmin
            loading={processing}
      ...
      <DialogConfirmAction
        ...
        title={t("confirm-update-quiz", { ns: NS_ADMIN_CHAPTERS })}
        labelConfirm={t("update-quiz", { ns: NS_ADMIN_CHAPTERS })}
